...
/* ADC 采样值缓冲区 */
/* 500 groups × 2 channels (CH10, CH12), filled by DMA in Normal mode */
uint16_t adc_value[1000];
/* set to 1 by DMA FTF interrupt when buffer is full, cleared by adc_task() */
volatile uint8_t adc_dma_done = 0;

static void timer2_adc_config(void);
...
void bsp_adc_init(void)
{
    ...
    dma_single_data_parameter.number = 1000;
    dma_single_data_parameter.priority = DMA_PRIORITY_HIGH;
    dma_single_data_mode_init(DMA1, DMA_CH0, &dma_single_data_parameter);
    dma_channel_subperipheral_select(DMA1, DMA_CH0, DMA_SUBPERI0);

    /* Normal mode (non-circular): DMA stops after 1000 transfers and fires FTF interrupt */
    ...
    /* single conversion per hardware trigger (timer-triggered mode) */
    ...
    /* ADC trigger config: TIMER2 TRGO rising edge (10kHz, PSC=119/ARR=99 @ 120MHz) */
    ...
    /* start TIMER2 to begin hardware-triggered ADC conversions */
    timer2_adc_config();
}

/* TIMER2: 120MHz / (119+1) / (99+1) = 10kHz TRGO for ADC triggering */
static void timer2_adc_config(void)
{
